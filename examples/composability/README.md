...
**Layout:** `workflows.ts` (bundle), **`run.ts`** — `worker` mode uses `createApp` + `createWorker`; **`demo`** uses **`createClient` only**. The task queue name is `durion-composability` in `run.ts`. For a client in another service, copy only the `demo` path; see [REMOTE_CLIENT.md](../REMOTE_CLIENT.md).
