...
**Layout:** `workflows.ts` (bundle), **`run.ts`** — `worker` mode uses `createApp` + `createWorker`; **`demo`** uses **`createClient` only**. Task queue defaults to **`TASK_QUEUE` env** or **`durion`**. For a client in another service, copy only the `demo` path; see [REMOTE_CLIENT.md](../REMOTE_CLIENT.md).
